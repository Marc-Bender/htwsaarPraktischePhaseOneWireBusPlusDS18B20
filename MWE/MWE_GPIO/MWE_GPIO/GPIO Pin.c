...
**/ 
/***
 *** implementierung von funktionen und interrupt handlern für die verwendung von gpio pins
***/
#include "GPIO Pin.h"
...
/***
 *** liest den zustand eines digitalen io pin (high / low) 
***/
/**
...
/***
 *** versetzt einen pin in den input modus (ohne internen pullup)
***/
/**
...
/***
 *** macht einen pin zum output und schalt ihn auf low. dies wird für die nach bildung von Opendrain auf AVR prozessoren gebraucht da dies nicht direkt möglich ist (im gegensatz zu z.b. stm32)
***/
/**
...
/***
 *** schaltet für einen pin den pullup aus. dies wird in der initialisierung des prozessors einmal verwendet um sicherzustellen, das WIRKLICH der pullup aus ist.
***/
/**
...
/***  hier werden die callbackfunktionen gespeichert die aufgerufen werden wenn ein externer interrupt ausgelöst wird ***/
...
/***
 *** diese funktion schaltet den externen interrupt an einem gewissen pin scharf. es wird nicht getestet ob der pin im speziellen überhaupt externe interrupts generieren kann.
***/
/**
...
/***
 *** diese funktion deaktiviert den externen interrupt in dem die entsprechenden bits zurück gesetzt werden und der entsprechende eintrag aus dem array mit callback funktionen gelöscht wird.
***/
/**
...
/***
 *** das ist der interrupt handler für EXTINT0. dieser interrupt wird dazu führen dass der prozessor sich aufhängt, wenn kein callback definiert ist sonst führt es diesen aus.
***/
/**
