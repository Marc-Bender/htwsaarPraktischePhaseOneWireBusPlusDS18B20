#include "UART.h"
/***
 *** Implementierung der funktionen des USART
***/
/**
...
**/
/***
 *** die baudraten einstellung benötigt kenntnis über den cpu takt. daher muss dieser definiert sein. ansonsten wird ein (sinniger?) default-wert angenommen.
***/
#ifndef F_CPU
...
/***
 *** FIFO Ringbuffer für zusendende Bytes
***/
static volatile GenericBuffers_t txBuffer; //< This is where the scheduled Characters / Strings are stored until they are actually sent. Since there is only one USART Channel supported right now (and only the transmitter thereof) one single variable for that purpose like shown here will work just fine.
...
**/
/***
 *** setzt die baudrate. Wenn ein unbekannter wert übergeben wird wird auf den 9600 baud modus zurückgegriffen. 
***/
...
**/ 
/***
 *** Diese funktion initialisiert den usart gerade genug um daten senden zukönnen. 
***/
...
**/
/***
 *** schaltet den interrupt für wenn das datenregister leer ist (ready to send) scharf. 
***/
...
**/
/***
 *** hebt die scharf schaltung des o.g. interrupts auf sodass nicht mehr daran errinnert wird wenn das daten register leer ist und in folge dessen gesendet werden könnte. 
***/
...
**/
/***
 *** Sendet einen string in den tx buffer und schaltet den daten register leer interrupt scharf, so dass so bald wie möglich die Daten über den USART gesendet werden.
 *** diese funktion ist verhältnismäßig dumm, daher sollte für alles was einen Konstanten Text übersteigt der Sendestring mit funktionen wie sprintf zusammen gepuzzelt werden und dann in
 *** den buffer übergeben werden, da diese funktion keine printf artigen argumente versteht. sie versteht nur einen pointer auf den datenbereich und die menge der zu sendenden daten.
***/
...
**/
/***
 *** Vergleichbar zu oben aber beschränkt auf das senden eines einzigen bytes an den tx buffer
***/
...
**/
/***
 *** Interrupt handler für den datenregister leer interrupt. sendet die bytes im tx buffer solange welche vorhanden. Wenn der tx buffer leer ist, wird der interrupt abgeschaltet.
***/
ISR(USART_UDRE_vect)
{
