...
**/
/***
 *** Implementierung der Funktionalität und der statemachines für die grundlegendsten One Wire Funktionalitäten. 
 *** Anmerkung im zugehörigen Header beachten!!!
***/

#include "OneWireLowLevel.h"

/*** Datenstrukuren zur internen Verwendung ***/ 
typedef struct
{
...
/*** interne Zustandsvariable für die Erkennung von Devices beim Presence Puls ***/
...
**/ 
/***
 *** Teil der Statemachine für das lesen eines einzelnen bits vom one wire bus.
 *** Wird indirekt per timer interrupt callback aufgerufen wenn die von der busspezifikation vorgegebene idle time nach dem lesen eines bits vergangen ist.
***/
...
**/ 
/***
 *** Startet die Statemachine für das lesen eines einzelnen bits vom one wire bus. 
 *** Wird direkt von der Funktionalität zum Lesen ganzer bytes vom bus aufgerufen. (s. FrontEnd-Code)
***/
...
**/ 
/***
 *** Teil der Statemachine für das schreiben einer logischen null auf den onewirebus. 
 *** Wird indirekt per timer interrupt aufgerufen, wenn der bus lange genug low war so dass per bus spezifikation alle slaves die null sicher erkannt haben müssen.
***/
...
**/ 
/***
 *** Startet die statemachine für das schreiben einer logischen null auf den bus. 
 *** Wird direkt von einer frontend funktion für das schreiben ganzer bytes auf den bus aufgerufen.
***/
...
**/ 
/***
 *** Teil der statemachine für das schreiben einer logischen eins auf den bus. 
 *** Wird indirekt aufgerufen wenn der bus per spezifikation von allen slaves gelesen (d.h. von allen slaves eine eins erkannt worden ist) 
***/
...
**/ 
/***
 *** startet die statemachine für das senden einer logischen eins über den bus. 
 *** wird direkt von einer frontend funktion für das schreiben eines ganzen bytes aufgerufen.
***/
...
**/ 
/***
 *** Teil der statemachine für das erkennen des presence pulses der einem reset folgen muss. Wird indirekt per timer interrupt aufgerufen wenn der timeslot in dem ein presence pulse auftreten muss
 *** vergangen ist. Ruft entweder den zuvor übergebenen callback auf oder hängt den prozessor auf wenn keine slaves vorhanden.
***/
...
**/ 
/***
 *** Teil der Statemachine für das erkennen des presence pulses. Zeigt an dass mindestens ein slave vorhanden ist.
 *** Wird indirekt per externem interrupt aufgerufen wenn die steigende flanke des presence pulse erkannt wird.
***/
...
**/ 
/***
 *** startet die state machine für die erkennung des presence pulses der dem reset puls zu folgen hat. 
***/
...
**/ 
/***
 *** letzte funktion in der state machine für das generieren eines reset pulses. wird indirekt per timer interrupt aufgerufen, wenn die mindestlänge der low phase eines reset pulses vergangen ist 
***/
...
**/ 
/***
 *** startet die statemachine für die generierung eines reset pulses. 
***/
...
**/ 
/***
 *** letzte funktion in der statemachine für das generieren der gesamten onewire initialisierungssequenz. wrapper funktion.
 *** wird indirekt per callback aufgerufen wenn der reset puls vollständig generiert wurde.
***/
 ...
 * this function may be moved to a more front end module since this is not exactly low level anymore.
**/ 
/***
 *** generiert die gesamte initialisierungssequence einer onewire übertragung (reset puls + presence pulse detection)
***/
