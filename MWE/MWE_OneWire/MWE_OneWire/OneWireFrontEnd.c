...
**/ 
/***
 *** implementierung für die funktionen der höheren Abstraktionsebene des onewire protokolls.
***/
...
/***
 *** Datentyp für die one wire bus kommandos die jeder slave verstehen muss.
***/
typedef enum
{
...
/***
 *** Interne Datenstrukur nicht unähnlich der im code für den ds18b20 temperatursensor
***/
typedef struct
{
...
/***
 *** diese funktion wird auf gerufen wenn das nächste byte der slave id gesendet werden soll. sie wird indirekt aufgerufen (als callback) wenn das schreiben des letzten bytes fertig ist.
...
/***
 *** diese funktion sendet die id über den bus die sie in einer datenstruktur für den internen gebrauch findet. sie wird direkt von einer funktion in einer übergeordeten statemachine
 *** oder indirekt aus dem callback des funktionsaufrufs der writebytetoonewirebus im inneren dieser funktion aufgerufen. Das sorgt für die quasi rekursive schleife (s. oben). 
 *** dieses vorgehen ist nötig da alle funktionalität timer getrieben ist.
 *** im übrigen die letzte funktion in der statemachine für die funktionalitäten in diesem modul die einen gewissen slave addressieren.
***/
/**
 ...
 *** sendet (was von der busspezifikation als nächster schritt nach dem senden des match rom kommandos vorgesehen wird)
***/
/**
 ...
 *** Sie sendet das Match Rom kommando über den bus. im übrigen ein wrapper wegen der bedingungen die in dieser code base an callback funktionen gestellt werden.
***/
/**
...
/***
 *** Diese funktion startet die statemachine die das matchrom kommand und die obligatorische id sendet. Weil Rom Commandos nicht beliebig gesendet werden können sorgt diese funktion auch dafür
...
/***
 *** Analog dazu für die Skip Rom seite der Welt
***/
/**
...
/***
 *** Analog dazu für das schreiben auf den Bus
***/
/**
